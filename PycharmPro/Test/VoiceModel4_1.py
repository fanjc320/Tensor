...
def main(param_mode,param_type,param_voice):
    ...
    if param_mode=='Train' and param_type=='FIR':
        K = tf.Variable(tf.zeros([GlottisTime]), dtype=tf.float32) 
        init_K_Ki = tf.assign(K,Ki)
    else:
    ...
    if param_mode=='Train' and param_type=='IIR' :
        Wbv = tf.Variable(tf.zeros([M]),dtype=tf.float32) 
        Wav = tf.Variable(tf.zeros([N]),dtype=tf.float32)
        init_W_Wai = tf.assign(Wav,Wai)
        init_W_Wbi = tf.assign(Wbv,Wbi)

    else:
        Wbv = tf.cast(Wbi,dtype=tf.float64)
        Wav = tf.cast(Wai,dtype=tf.float64)
        
    Wb = tf.cast(tf.reshape(Wav,[M,1]),dtype=tf.float64) 
    Wa = tf.cast(tf.reshape(Wbv,[N,1]),dtype=tf.float64)         
    ...
    wa_filepath = model_folder + 'wa.dat'
    wb_filepath = model_folder + 'wb.dat'
    if os.path.exists(os.path.abspath(wa_filepath)):
        LoadData(wa_filepath,wa_data)
    if os.path.exists(os.path.abspath(wb_filepath)):
        LoadData(wb_filepath,wb_data) 
    
    #从文件导入k
    glot_wav = np.zeros([GlottisTime],dtype='int16') 
    #m_filepath = model_folder + 'm74.snd'.format(GlottisTime) 
    #LoadWave(m_filepath,0,glot_wav,GlottisTime)  
    k_filepath = model_folder + 'k{}.snd'.format(GlottisTime)
    if os.path.exists(os.path.abspath(k_filepath)):
        LoadWave(k_filepath,0,glot_wav,GlottisTime)      
            
    k_data = glot_wav[::-1].astype(np.float32) #TF卷积核是颠倒的
    ...
    with tf.Session() as sess:
        ...
        if param_mode=='Train' and param_type=='FIR':
            sess.run(init_K_Ki,feed_dict=feed)
        
        if param_mode=='Train' and param_type=='IIR':
            sess.run(init_W_Wai,feed_dict=feed)    
            sess.run(init_W_Wbi,feed_dict=feed)    
        ...
        while(param_mode == 'Train'):
            ...
            if iteration % 2000 == 0:
                #保存Wa,Wb到文件
                if param_mode=='Train' and param_type=='IIR' :
                    wav = sess.run(Wav, feed_dict=feed).astype(np.float32)
                    wbv = sess.run(Wbv, feed_dict=feed).astype(np.float32)
                    SaveData(wa_filepath,wav)
                    SaveData(wb_filepath,wbv)
                    
                #保存k到文件
                if param_mode=='Train' and param_type=='FIR' :
                    k = sess.run(K, feed_dict=feed)
                    glot_wav = k[::-1].astype(np.int16)
                    SaveWave(k_filepath,glot_wav,1,False)
        ...
        #波形显示   
        show_one(glot_wav,'b') #声门波
        show_one(wave_y_base,'r')#目标元音
        show_one(wave_pred,'y') #元音预测
        ...
        for n in range(30):                        
            ...
            SaveWave('../data/gen_{}_{}.snd'.format(param_voice,GlottisTime),wave_pred,5,bAppend)
            bAppend = True
